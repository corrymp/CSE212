...
public static class Search {
    ...
    private static int SearchSorted1(int[] data, int target) { // O(n): goes over every number in order till it finds it
        var count = 0;
    ...
    private static int SearchSorted2(int[] data, int target, int start, int end) { // O(log n): starts in the middle and keeps halving the search region until it finds it
        if (end < start)
            return 1; // All done
